...
<div class="row" style="background-color:#eee;padding:20px; border-radius:20px;margin-bottom:10px;">
    ...
    <form method="post" action="{{route("yonetim.kullanici.arama")}}">
    ...
    <table class="table table-hover table-bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$entry->adsoyad}}</td>
                <td>{{$entry->email}}</td>
                <td>{!! ($entry->aktif_mi == 1) ? '<span class="btn btn-success btn-xs">Aktif</span>' : '<span
                        class="btn btn-warning btn-xs">Pasif</span>' !!}</td>
                <td>{!! ($entry->yoneticimi == 1) ? '<span class="btn btn-success btn-xs">Yönetici</span>' : '<span
                        class="btn btn-info btn-xs">Kullanıcı</span>' !!}</td>
